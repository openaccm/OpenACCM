...
class ManaDashboardConfigBase(models.Model):
    ...
    def _default_name(self):
        ...
        default name
        """
        raise exceptions.NotImplementedError('The _default_name method must be implemented!')
        ...
        if options.get('fetch_data'):
            ...
            if self.multi_data_source:
                for data_source in self.data_source_ids:
                    data_sources.append(data_source.get_datas(options))
            else:
                # inherit from mana_data_source
                data_sources.append(self.get_datas(options))
