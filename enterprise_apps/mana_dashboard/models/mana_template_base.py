...
class ManaDashboardTemplateBase(models.Model):
    ...
    @api.onchange('history_data')
    def onchange_history_data(self):
        '''
        onchange history data, as to avoid discard the changes
        '''
        if self.template or self.styles or self.scripts:
            self.save_history_data()

        # load form history data
        history_data = json.loads(self.history_data.data)

        # restore history data
        self.restore_history_data(history_data)

    def restore_history_data(self, history_data):
        """
        Restore History Data,
        """
        # just the item we want to change
        self.template = history_data['template']
        self.styles = history_data['styles']
        self.scripts = history_data['scripts']
